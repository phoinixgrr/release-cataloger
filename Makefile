...
DOCKER_REGISTRY_REPO    ?= phoinixgrr/${APP_NAME}-daily
...
## Docker Images
DOCKER_IMAGE_GO         ?= "golang:${GO_VERSION}@sha256:f020456572fc292e9627b3fb435c6de5dfb8020fbcef1fd7b65dd092c0ac56bb"
...
# LDFLAGS
GO_LDFLAGS += -X "main.gitVersion=$(GIT_VERSION)"
GO_LDFLAGS += -X "main.buildHash=$(BUILD_HASH)"
GO_LDFLAGS += -X "main.buildTagLatest=$(BUILD_TAG_LATEST)"
GO_LDFLAGS += -X "main.buildTagCurrent=$(BUILD_TAG_CURRENT)"
GO_LDFLAGS += -X "main.gitTreeState=$(GIT_TREESTATE)"
GO_LDFLAGS += -X "main.buildDate=$(BUILD_DATE)"

# Architectures to build for
GO_BUILD_PLATFORMS           ?= linux-amd64 linux-arm64 freebsd-amd64 darwin-amd64 darwin-arm64
...
package-software:  ## to package the binary
	@$(INFO) Packaging
	$(AT) for file in $(GO_OUT_BIN_DIR)/release-cataloger-*; do \
		...
		mv $$file $(GO_OUT_BIN_DIR)/$${target}_temp/bin/release-cataloger; \
